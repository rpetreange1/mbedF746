...
SDMMCBlockDevice::SDMMCBlockDevice(PinName cardDetect) : _cardDetect(cardDetect, PullUp),
                                                       _is_initialized(0),
                                                       _sectors(0),
                                                       _init_ref_count(0)
{
...
int SDMMCBlockDevice::read(void *buffer, bd_addr_t addr, bd_size_t size)
{
    ...
    debug_if(SD_DBG, "ReadBlocks dbgtest addr: %lld  blockCnt: %lld status: %d\n", addr, blockCnt, status);
...
bool SDMMCBlockDevice::_is_valid_trim(bd_addr_t addr, bd_size_t size)
{
    return (
        addr % _erase_size == 0 &&
        size % _erase_size == 0 &&
...
bool SDMMCBlockDevice::isPresent(void)
{
